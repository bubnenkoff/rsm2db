...
import std.algorithm : min, max, countUntil, canFind;
import std.conv   : to;
...
struct IniSection
{
    ...
    private void parse(const(char)[] line, size_t lineCount, ref IniSection* section)
    {
        ...
        if(line.idup.canFind("::"))  return;
        if(line.idup.canFind(".rsm"))  return; // some stupid strings in head
    ...
    if(isSomeString!(T) && isSomeString!(S))
    {
        ...
        parts ~= txt[last .. txt.length].split(";")[0];       
